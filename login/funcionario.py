...
class funcionarioDao:
    ...
    def salvar(self, f):
        ...
        if (verifica):
                con = psycopg2.connect(self._conexao)
                cursor = con.cursor()
                cursor.execute('UPDATE Funcionario SET nome = %s, email = %s, idDepartamento = %s, login = %s, senha=%, admin=FALSE WHERE idFuncionario = %s',(f.nome,f.email,f.departamento,f.login,f.senha,int(f.id)))
                con.commit()
                cursor.close()
